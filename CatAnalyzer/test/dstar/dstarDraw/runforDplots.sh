...
cut_d0='tri!=0&&filtered==1&&is3lep==2&&d0_L3D>0.2&&d0_LXY>0.1' #&&abs(d0.M()-1.8648)<0.040'
cut_dstar='tri!=0&&filtered==1&&is3lep==2&&d0_L3D>0.2&&d0_LXY>0.1&&dstar_L3D>0.2&&dstar_LXY>0.1' #&&abs(dstar_diffMass-0.145)<0.01'
...
./dstarDraw.py -c "${cut_d0}" -t $treename -s 5 -b [20,1.6,2.2] -p d0.M\(\) -x 'Mass of D0 [GeV/c^{2}]' > /dev/null &
